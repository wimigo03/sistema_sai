        ...
        <tbody>
            @foreach($categorias_programaticas as $categoria_programatica)
                <thead class="border-bottom">
                    <tr>
                        <td align="left" colspan="2"><b>CAT. PROG.</b></td>
                        <td align="left" colspan="3">{{ $categoria_programatica->codigo . ' - '. $categoria_programatica->nombre }}</td>
                        <td align="right"><b>SUB TOTAL</b></td>
                        <td align="right"><b>{{ $categoria_programatica->total_categorias_programaticas }}</b></td>
                    </tr>
                </thead>
                <tr>
                    <td colspan="7">&nbsp;</td>
                </tr>
                @foreach($partidas_presupuestarias->where('categoria_programatica_id', $categoria_programatica->categoria_programatica_id) as $partida_presupuestaria)
                    <thead class="border-bottom">
                        <tr>
                            <td align="left" colspan="2"><b>PART. PRES.</b></td>
                            <td align="left" colspan="3">{{ $partida_presupuestaria->codigo . ' - '. $partida_presupuestaria->nombre }}</td>
                            <td align="right"><b>SUB TOTAL</b></td>
                            <td align="right"><b>{{ $partida_presupuestaria->total_partidas_presupuestarias }}</b></td>
                        </tr>
                    </thead>
                    <thead class="border-bottom">
                        <tr>
                    ...
                    @foreach($ingresos_compras_detalles->where('partida_presupuestaria_id', $partida_presupuestaria->partida_presupuestaria_id) as $datos)
                        <tr>
                            <td align="center">{{ $cont++ }}</td>
                            <td align="center">{{ $datos->item->codigo }}</td>
                            <td>{{ $datos->item->nombre }}</td>
                            <td align="center">{{ $datos->item->unidad_medida->alias }}</td>
                            <td align="right">{{ number_format($datos->cantidad,2,'.',',') }}</td>
                            <td align="right">{{ number_format($datos->orden_compra_detalle->precio,2,'.',',') }}</td>
                            <td align="right">{{ number_format($datos->cantidad * $datos->orden_compra_detalle->precio,2,'.',',') }}</td>
                        </tr>
                    @endforeach
                    <br>
                @endforeach
