    ...
    <div class="col-md-10">
        ...
        <div class="row font-verdana-bg">
            ...
            <div class="col-md-8 text-right titulo">
                <b>EDITAR DETALLE DE INGRESO ESTADO 6</b>
            </div>
        ...
        <div class="body-border">
            <font size="2" face="Courier New" >
                    <form method="POST" action="{{ route('comegreso.updatearchivonotaseis',$docproveedor->iddetallecomegreso) }}" id="form">
                        @csrf
                                ...
                                <select name="iddetallecomingreso" id="iddetallecomingreso" placeholder="--Seleccionar--"
                                    ...
                                    @foreach ($detallecoming as $area)
                                        @if ($area->iddetallecomingreso == $docproveedor->iddetallecomingreso)
                                            <option value="{{ $area->iddetallecomingreso }}" selected>CODIGO: {{ $area->detalleprodcomb }} //NOMBRE: {{ $area->nombreprodcomb }} //UNIDAD: {{ $area->nombremedida }} //BS. {{ $area->precio }} //SALDO: {{ $area->difcantidad }}</option>
                                        @else
