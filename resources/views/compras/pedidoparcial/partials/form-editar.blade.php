<div class="card card-body bg-light">
    ...
    <div class="form-group row font-verdana-bg">
        <div class="col-md-3 pr-1">
            ...
            <input type="text" name="preventivo" value="{{ $compra->preventivo }}" class="form-control form-control-sm font-verdana-bg" id="preventivo" onkeypress="return valideNumberInteger(event);">
        </div>
        <div class="col-md-2 pl-1">
            <label for="f_preventivo" class="d-inline">Fecha Preventivo</label>
            <input type="text" name="fecha_preventivo" value="{{ $compra->fecha_preventivo != null ? \Carbon\Carbon::parse($compra->fecha_preventivo)->format('d/m/Y') : '' }}" placeholder="dd/mm/aaaa" class="form-control form-control-sm font-verdana-bg" id="fecha_preventivo" data-language="es" autocomplete="off">
        </div>
    </div>
    <div class="form-group row font-verdana-bg">
        <div class="col-md-12">
            <label for="objeto" class="d-inline">Objeto <i class="fa-solid fa-xs fa-asterisk"></i></label>
            <textarea name="objeto" cols="1" rows="1" class="form-control form-control-sm font-verdana-bg" id="objeto" oninput="this.value = this.value.toUpperCase()">{{ $compra->objeto }}</textarea>
        </div>
    </div>
    <div class="form-group row font-verdana-bg">
        <div class="col-md-12">
            <label for="justificacion" class="d-inline">Justificacion <i class="fa-solid fa-xs fa-asterisk"></i></label>
            <textarea name="justificacion" cols="1" rows="1" class="form-control form-control-sm font-verdana-bg" id="justificacion" oninput="this.value = this.value.toUpperCase()">{{ $compra->justificacion }}</textarea>
        </div>
    </div>
</div>