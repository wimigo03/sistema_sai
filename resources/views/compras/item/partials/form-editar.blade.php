<div class="card-body body">
    <form action="#" method="post" id="form">
        @csrf
        <input type="hidden" name="dea_id" value="{{ $item->dea_id }}">
        ...
        <div class="form-group row font-roboto-12 abs-center">
            <div class="col-md-5 pr-1 pl-1">
                ...
                <select name="partida_id" id="partida_id" class="form-control select2">
                    @foreach ($partidas_presupuestarias as $partida_presupuestaria)
                        <option value="{{ $partida_presupuestaria->id }}"
                            @if($partida_presupuestaria->id == old('partida_id') || (isset($item) && $item->partida_id == $partida_presupuestaria->id))
                                selected
                            @endif>
                            {{ $partida_presupuestaria->nombre }}
                        </option>
                    @endforeach
                </select>
            </div>
