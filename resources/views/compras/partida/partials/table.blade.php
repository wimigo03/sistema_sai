        ...
        <table class="table display table-striped table-bordered responsive hover-orange" style="width:100%;">
            <thead>
                <tr class="font-roboto-11">
                    ...
                    @canany(['partida.habilitar','partida.editar'])
                                    ...
                                    @can('partida.habilitar')
                                        ...
                                        @else
                                            <span class="tts:left tts-slideIn tts-custom" aria-label="Habilitar" style="cursor: pointer;">
                                                <a href="{{ route('partida.habilitar',$datos->id) }}" class="badge-with-padding badge badge-success">
            ...
            <tfoot>
                <tr class="font-roboto-11">
                    <td colspan="12">
                        {{ $partidas->appends(Request::all())->links() }}
                        <p class="text- muted">Mostrando
                            <strong>{{$partidas->count()}}</strong> registros de
                            <strong>{{$partidas->total()}}</strong> totales
                        </p>
                    </td>
                </tr>
            </tfoot>
