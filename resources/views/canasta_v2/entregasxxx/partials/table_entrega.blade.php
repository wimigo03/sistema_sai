                ...
                @foreach ($entrega as $entregas)
                    <tr class="font-roboto-11">
                        ...
                        <td class="text-center p-1">
                            <img src="{{ asset(substr($entregas->dir_foto, 3)) }}" align="center" height="30" with="30" />
                        </td>
                        <td class="text-center p-1">
                            <span class="{{ $entregas->colorStatus }}">
                                @if ($entregas->estado == 1)
                                    <span class="fa-stack fa-sm">
                                        <i class="fa-solid fa-print fa-stack-1x"></i>
                                        <i class="fa-solid fa-ban fa-stack-2x" style="color:Tomato"></i>
                                    </span>&nbsp;{{ $entregas->status }}
                                @endif
                                @if ($entregas->estado == 2)
                                    <a href="{{ route('entregas.generarboleta2', $entregas->id) }}" class="text-dark" target="_blank">
                                        <i class="fas fa-print fa-fw"></i>&nbsp;{{ $entregas->status }}
                                    </a>
                                @endif
                                @if ($entregas->estado == 3)
                                    <i class="fas fa-user-check fa-fw"></i>&nbsp;{{ $entregas->status }}
                                @endif
                            </span>
                        </td>
                        <td class="text-center p-1">
                            ...
                            @if ($entregas->estado == 3 && $estados == 3)
                                <span class="tts:left tts-slideIn tts-custom" aria-label="Quitar Entrega" style="cursor: pointer;">
                                    <a href="{{ route('entregas.deshabilitar', [$entregas->id, $entregas->paquete->id]) }}" class="badge-with-padding badge badge-danger">
                                        <i class="fa-regular fa-circle-down fa-fw"></i>
                                    </a>
                                </span>
                            @elseif ($entregas->estado == 2 && $estados == 3)
                                <span class="tts:left tts-slideIn tts-custom" aria-label="Entregar" style="cursor: pointer;">
                                    <a href="{{ route('entregas.habilitar', [$entregas->id, $entregas->paquete->id]) }}" class="badge-with-padding badge badge-success">
