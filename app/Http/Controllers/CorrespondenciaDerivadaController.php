...
class CorrespondenciaDerivadaController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       //dd($personal->nombrearea);
        if ($request->ajax()) {
            $data = DB::table('deriv_corresp as d')
                      ->join('recepcion as r', 'r.id_recepcion', 'd.id_recepcion')
                        ...
                        ->where('a.idarea', $contratos->idarea_asignada)
                        ->select('d.fechaderivacion','r.estado_corresp',
                                    'r.id_recepcion','d.idderivacion',
                                    'i.nombreinstruccion','r.asunto',
                                    'a.nombrearea','d.created_at')

                        ->orderBy('d.idderivacion', 'desc');

            return Datatables::of($data)->addIndexColumn()
                                        ->addColumn('btn', 'correspondencia-derivada.btn')
                                        ->rawColumns(['btn'])
                                        ->make(true);
        }

        return view('correspondencia-derivada.index', ["area" => $personal->nombrearea]);
    }
    ...
    public function gestionarCorrespondencia($idrecepcion)
    {
        ...
        return view('correspondencia-derivada.gestionarCorrespondencia', ["data" => $data]);
    }
       ...
       // dd($personalArea->idarea);
        ...
        public function respuesta(Request $request)
        {
            ...
            if ($request->ajax()) {
               ...
               ->join('areas as a', 'a.idarea', '=', 'd.idarea')
               ->select('d.idarea','d.idderivacion','d.estadoderiv1')
               ->where('d.idarea', $contratos->idarea_asignada)
               ->where('d.estadoderiv1', 1)
